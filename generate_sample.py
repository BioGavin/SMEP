...
class GenerateSample():
    ...
    def __call__(self, *args, **kwargs):
        ...
        # data = self.positive_data_filter(data)  # Edited by Gavin.
    ...
    def positive_data_filter(self, data):
        """Added by Gavin."""
        data = data[data["has_cterminal_amidation"] == True]  # 1. Standardized c-terminal amidation filtering
        data = data[data["sequence"].apply(lambda x: 5 <= len(x) <= 50)]  # 2. Sequence length: 5~50
        return data
    ...
    def generate_negative_data(self, negative_file_path):
        ...
        data_negative = data_negative[data_negative["Sequence"].apply(lambda x: len(x) < 40)]
